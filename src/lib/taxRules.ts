...
export const TAX_RULES: TaxRule[] = [
  ...
  },
  {
    ...
    notes:
      ...
      "LIMITES CONNUES DU MODÈLE, à corriger manuellement le cas échéant :\n" +
      "— Véhicule d'occasion acheté à un particulier ou sous le régime de la marge : le prix ne contient aucune TVA récupérable. Le modèle suppose au contraire un prix TTC grevé de TVA déductible ; ne pas activer l'option dans ce cas.\n" +
      "— Levée de l'option d'achat en LOA : le prix de rachat est un paiement unique, affiché hors coût annuel par le simulateur ; la TVA qu'il porte n'est donc pas prise en compte alors qu'elle serait en principe récupérable dans les mêmes conditions.\n" +
      "— Offre LLD « tout compris » : si le loyer inclut déjà l'entretien, laisser un montant d'entretien saisi le ferait compter deux fois dans la base de TVA comme dans le coût — mettre le champ à 0, comme le rappelle déjà l'avertissement du mode LLD.\n\n" +
